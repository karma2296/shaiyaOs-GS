...
const DiscordLogin: React.FC<DiscordLoginProps> = ({ onLogin, onBack }) => {
  ...
  // Re-chequear configuración al montar el componente
  useEffect(() => {
  ...
  const handleLoginAction = async () => {
    ...
    // Si detectamos las llaves o si el usuario quiere intentar de todos modos
    if (configured && supabase) {
      try {
        ...
        alert(`Supabase Error: ${err.message || 'Check your Discord Provider settings in Supabase Dashboard.'}`);
        setLoading(false);
      }
    } else {
      // Si realmente no hay llaves, activamos simulación
      console.warn("Keys not found in process.env. Entering simulation.");
      runSimulation();
    }
  };
  ...
  return (
    <div className="max-w-md mx-auto w-full glass-morphism p-10 rounded-none border-t-2 border-t-[#c5a059] text-center shadow-[0_0_100px_rgba(0,0,0,0.8)]">
      <div className="w-20 h-20 bg-[#5865F2] rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-[0_0_30px_rgba(88,101,242,0.4)] relative">
        ...
        {configured && <div className="absolute -top-1 -right-1 w-4 h-4 bg-emerald-500 rounded-full border-2 border-black animate-pulse"></div>}
      </div>
      
      <h2 className="text-2xl font-fantasy gold-text mb-4 uppercase tracking-[0.2em]">Council Verification</h2>
      
      {!configured ? (
        <div className="bg-amber-900/20 border border-amber-900/50 p-4 mb-8 text-left">
          <p className="text-[11px] text-amber-200 uppercase font-bold mb-1 tracking-widest">⚠️ Detection Pending</p>
          <p className="text-[10px] text-slate-400 font-gothic leading-tight">
            Environment variables not yet active. If you just added them, please <b>refresh the page</b> or wait a few moments.
          </p>
        </div>
      ) : (
        <div className="bg-emerald-900/10 border border-emerald-900/30 p-3 mb-8">
           <p className="text-[10px] text-emerald-400 uppercase font-gothic tracking-widest">✨ Cloud Link Established</p>
        </div>
      )}

      <p className="text-slate-500 mb-10 font-gothic text-sm leading-relaxed max-w-xs mx-auto">
        Verify your identity through the Discord Archives to access the recruitment scrolls.
      </p>
      ...
      <div className="space-y-4">
        ...
        >
          {loading ? (
            ...
            <>
              <span>{configured ? "Login with Discord" : "Try Manual Login"}</span>
